...
Source `ac4b0c2b` was built for aarch64-macos and x86_64-linux-gnu and vendored
in Granis `1cc0687f9` with provenance/delivery acceptance (80 s). The actual
native read → tool-result history → commit proof passes: two scripted requests,
zero post-terminal requests, normal exit (27 ms). Four-turn mechanics use four
requests per provider (5 s supervisor). These are mechanics, not model quality.
Granis owns the pending real-provider comparison and production release; its
receipt is `docs/reviews/evidence/2026-09-04-native-head-qualification/terminal-ack-mechanics-01/acceptance.json`.
Agent Central N8 records the candidate acceptance rule without changing its
signed runtime pin. Preserve foreign worktrees; their commits are on main.
Upstream fetched before this change: fork main21ahead/0behind.
